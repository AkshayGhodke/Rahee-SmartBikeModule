...
{
    //u0_dbg_printf("inside handleInterrupt");
    /* If transfer finished (not busy), then give the signal */
    if (busy != i2cStateMachine()) {
        //u0_dbg_printf("inside handleInterrupt's IF \n");
...
bool I2C_Base::initSlave(uint8_t slaveAddress, volatile uint8_t * buffer1, uint32_t size){
    mpI2CRegs->I2ADR0 = slaveAddress;
    u0_dbg_printf(" ConSet Value b4 : %x \n",mpI2CRegs->I2CONSET);
    mpI2CRegs->I2CONSET |= ((1 << 2) |(1<<6));
    u0_dbg_printf(" ConSet Value after : %x \n",mpI2CRegs->I2CONSET);
    mSlaveTransaction.pSlaveData = buffer1;
    mSlaveTransaction.firstReg = buffer1;
    mSlaveTransaction.trxSize = size;
...
I2C_Base::mStateMachineStatus_t I2C_Base::i2cStateMachine()
{
    ...
    //u0_dbg_printf(" Start of State current StateEndW : %x \n",mpI2CRegs->I2STAT);
    enum {
        // General states :
        busError = 0x00,
        start = 0x08,
        ...
        stopRepeatStartCondition = 0xA0,

        // Slave Receiver States
        //slaveAddressReceivedW = 0x60,
        slaveAddressRecognized = 0x60,
        slaveAddDataReceivedAckReturned = 0x80,
        slaveAddDataReceivedNackReturned = 0x88,

        // Slave Transmitter States:

        slaveAddressReceivedR = 0xA8,
        slaveAddDataTransmitedAckReceived = 0xB8,
        slaveAddDataTransmitedNackReceived = 0xC0,
        lastDataByteTransmitedAckReceived = 0xC8,
...
#define setStop()           clearSTARTFlag();                           \
                                ...
                                if(I2C_READ_MODE(mTransaction.slaveAddr))   \
                                    ...
                                    state = writeComplete;
    static bool regFlag = true;
    //u0_dbg_printf(" current State : %x \n",mpI2CRegs->I2STAT);
    switch (mpI2CRegs->I2STAT) {
        case start:
            mSlaveTransaction.pSlaveData = mSlaveTransaction.firstReg;
            //mSlaveTransaction.trxSize = 256;
            regFlag = true;
        ...
        case arbitrationLost:
            ...
            // Added code for slave

        case stopRepeatStartCondition:
            u0_dbg_printf("inside slaveAddressReceivedW Stat : %x \n", mpI2CRegs->I2STAT);
            u0_dbg_printf("inside slaveAddressReceivedW Data : %x \n", mpI2CRegs->I2DAT);
            //setSTARTFlag();
            //mSlaveTransaction.pSlaveData = mSlaveTransaction.firstReg;
            regFlag = true;
            setAckFlag();
        ...
        case slaveAddressRecognized:

            u0_dbg_printf("inside slaveAddressRecognized Stat : %x \n", mpI2CRegs->I2STAT);
            u0_dbg_printf("inside slaveAddressRecognized Data : %x \n", mpI2CRegs->I2DAT);

            setAckFlag();
            clearSIFlag();
            break;
        case slaveAddDataReceivedAckReturned:
            u0_dbg_printf("inside slaveAddDataReceivedAckReturned Stat : %x \n", mpI2CRegs->I2STAT);
            u0_dbg_printf("inside slaveAddDataReceivedAckReturned Data : %x \n", mpI2CRegs->I2DAT);
            if (regFlag) {
                regFlag = false;
                u0_dbg_printf(" Register identified \n");
                mSlaveTransaction.memIndex = mpI2CRegs->I2DAT;
                //mSlaveTransaction.trxSize -= mSlaveTransaction.memIndex;
                setAckFlag();
            }
            else if (!regFlag) {
                //u0_dbg_printf(" else ++++++++++ \n");
                *(mSlaveTransaction.pSlaveData + (mSlaveTransaction.memIndex - 1)) = mpI2CRegs->I2DAT;
                //*mSlaveTransaction.pMasterData = mpI2CRegs->I2DAT;
                u0_dbg_printf(" Data ReceivedW : %x, %x \n", (mSlaveTransaction.pSlaveData + (mSlaveTransaction.memIndex - 1)),
                        *(mSlaveTransaction.pSlaveData + (mSlaveTransaction.memIndex - 1)));
                //++mSlaveTransaction.pSlaveData;
                mSlaveTransaction.memIndex = (mSlaveTransaction.memIndex + 1) % 0xFF;
                setAckFlag();
            }
            else {
                mSlaveTransaction.memIndex = mpI2CRegs->I2DAT;
            ...
            break;
        case slaveAddDataReceivedNackReturned:
            u0_dbg_printf("inside slaveAddDataReceivedNackReturned Stat : %x \n", mpI2CRegs->I2STAT);
            u0_dbg_printf("inside slaveAddDataReceivedNackReturned Data : %x \n", mpI2CRegs->I2DAT);
        ...
        case slaveAddressReceivedR:
            u0_dbg_printf("inside slaveAddressReceivedR Stat : %x \n", mpI2CRegs->I2STAT);
            u0_dbg_printf("inside slaveAddressReceivedR Data : %x \n", mpI2CRegs->I2DAT);
            u0_dbg_printf("inside slaveAddressReceivedR Index  : %x \n", mSlaveTransaction.memIndex);

            if (mSlaveTransaction.memIndex <= 0xFF) {
                mpI2CRegs->I2DAT = *(mSlaveTransaction.pSlaveData + (mSlaveTransaction.memIndex - 1));

                u0_dbg_printf(" current StateR : %x \n", mpI2CRegs->I2STAT);
                u0_dbg_printf(" Data Sent1 : %x %x \n", (mSlaveTransaction.pSlaveData + (mSlaveTransaction.memIndex - 1)),
                        *(mSlaveTransaction.pSlaveData + (mSlaveTransaction.memIndex - 1)));
                //++mSlaveTransaction.pSlaveData;
                mSlaveTransaction.memIndex = (mSlaveTransaction.memIndex + 1) % 0xFF;
                setAckFlag();
            ...
            clearSIFlag();
            break;
        case slaveAddDataTransmitedAckReceived:

            u0_dbg_printf("inside slaveAddDataTransmitedAckReceived : %x \n", mpI2CRegs->I2STAT);
            u0_dbg_printf("inside slaveAddDataTransmitedAckReceived : %x \n", mpI2CRegs->I2DAT);
        ...
        case lastDataByteTransmitedAckReceived:
            u0_dbg_printf("inside lastDataByteTransmitedAckReceived Stat : %x \n", mpI2CRegs->I2STAT);
            u0_dbg_printf("inside lastDataByteTransmitedAckReceived Data : %x \n", mpI2CRegs->I2DAT);
        ...
        case slaveAddDataTransmitedNackReceived:
            u0_dbg_printf("inside slaveAddDataTransmitedNackReceived Stat : %x \n", mpI2CRegs->I2STAT);
            u0_dbg_printf("inside slaveAddDataTransmitedNackReceived Data : %x \n", mpI2CRegs->I2DAT);
