...
class I2C_Base
{
    public:
        ...
        bool initSlave(uint8_t, volatile uint8_t *, uint32_t );
    ...
    protected:
         ...
         * @param busRateInKhz  The speed to set for this I2C Bus
         */
         //bool initSlave(uint32_t , uint32_t , uint32_t );
    ...
    private:
        ...
        typedef struct
        {
            uint32_t trxSize;   ///< # of bytes to transfer.
            uint8_t slaveAddr;  ///< Slave Device Address
            volatile uint8_t *firstReg;   ///< 1st Register to Read or Write
            uint8_t error;      ///< Error if any occurred within I2C
            volatile uint8_t *pSlaveData;  ///< Buffer of the I2C Read or Write
            uint8_t memIndex;     ///< memory index to be written
        } mI2CSlaveTransaction_t;

        /// The I2C Input Output frame that contains I2C transaction information
        mI2CSlaveTransaction_t mSlaveTransaction;
        ...
        typedef enum {
            ...
            slaveReadComplete,
            slaveWriteComplete,
            slaveAddressrecognized
        } __attribute__((packed)) mSlaveStateMachineStatus_t;


        mSlaveStateMachineStatus_t i2cSlaveStateMachine();
