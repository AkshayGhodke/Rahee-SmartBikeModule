...
//extern TaskHandle_t xHandle = NULL;
...
// Sends direction to approach
CMD_HANDLER_FUNC(direct);

// Send type to blink or not
CMD_HANDLER_FUNC(blinky);

CMD_HANDLER_FUNC(brightnessLevel);

/// Send a cmd command to control a task
CMD_HANDLER_FUNC(orientationCmd);

// Send cmd command to Suspend or Resume tasks
