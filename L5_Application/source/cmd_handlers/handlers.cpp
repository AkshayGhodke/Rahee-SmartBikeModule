...
CMD_HANDLER_FUNC(i2cIoHandler)
{
    ...
    /*else if (write) {
     ...
     output.printf("Error writing to device %#x\n", addr);
        ...
        //Added 3 more data values

        uint8_t dataVal[4]; //AL

        uint32_t transSize = 4; //AL

        if (6 < cmdParams.scanf("%*s %0x %0x %0x %0x %0x %0x ", &addr,

        &reg, &dataVal[0], &dataVal[1], &dataVal[2], &dataVal[3])) {
        ...
        //Calling writeRegisters

        if (I2C2::getInstance().writeRegisters(addr, reg, dataVal, transSize)) {
...
CMD_HANDLER_FUNC(taskStateHandler)

{
    if(cmdParams == "resume1"){
        vTaskResume(xHandle1);
    }
    if(cmdParams == "suspend1"){
       vTaskSuspend(xHandle1);
    }
    if(cmdParams == "resume2"){
        vTaskResume(xHandle2);
    }
    if(cmdParams == "suspend2"){
       vTaskSuspend(xHandle2);
    }
    return true;
}

CMD_HANDLER_FUNC(bikeLockStatus)
{
        ...
        if(cmdParams == "unlock"){
            printf("unLocked");
            flag_bike_lock = false;
                }
        return true;
}

CMD_HANDLER_FUNC(direct){
    /*if (cmdParams == "0") {
                direction = 0;
    }
    if (cmdParams == "1") {
