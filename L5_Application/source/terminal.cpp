...
bool terminalTask::taskEntry()
{
    ...
    cp.addHandler(taskStateHandler, "task", "Toggle Task1 or Task2 State");

    // Added by Akshay

    cp.addHandler(bikeLockStatus, "bikeLockStatus", "options: true or false");
    cp.addHandler(direct, "direct", "0, 1, 2 or 3 ");
    cp.addHandler(blinky, "blinky", "0, 1, 2 or 3 ");
    //cp.addHandler(brightnessLevel, "brightnessLevel", "0, 1, 2 or 3 ");


    //scp.addHandler(i2cBufferIoHandler,   "i2cbuffer",   "'i2cbuffer count ' : Reads <count> registers of device 0x01 starting from 0x02\n"
